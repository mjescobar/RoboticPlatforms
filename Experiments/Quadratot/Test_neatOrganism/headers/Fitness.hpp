...
#define F_DISTANCE_PENALIZATION(X) (double)exp(-pow((X - PERFECT_FRECUENCY)/GAUSSIAN_STDD, 2.0)/2.0)*AMPLITUDE/sqrt(2*Pi*GAUSSIAN_STDD*GAUSSIAN_STDD) + 0.000001

#define FITNESS_FUNCTION(X) (double)exp(-pow((X - DISTANCE_OBJETIVE)/GAUSSIAN_STDD, 2.0)/2.0)*AMPLITUDE/sqrt(2*Pi*GAUSSIAN_STDD*GAUSSIAN_STDD) + 0.000001
